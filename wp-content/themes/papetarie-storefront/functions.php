    ...
    foreach ($settings as $setting) {
        ...
        if (!empty($setting['id']) && $setting['id'] === 'woocommerce_price_thousand_sep') {
            $augmented_settings[] = [
                ...
                'type' => 'number',
                'default' => '50',
                'desc' => __('Order subtotal before discounts, excluding shipping and taxes. Set in RON.', 'papetarie-storefront'),
                'desc_tip' => true,
                'custom_attributes' => [
                    'min' => '0',
                    'step' => '0.01',
...
function papetarie_storefront_shipping_method_label_text(WC_Shipping_Rate $rate): string
{
    $label = trim((string) $rate->get_label());
    if ($label === '') {
        $label = trim((string) wc_cart_totals_shipping_method_label($rate));
    }
    return trim(wp_strip_all_tags(html_entity_decode($label, ENT_QUOTES, 'UTF-8')));
}

function papetarie_storefront_shipping_method_price_text(float $amount): string
{
    return trim(wp_strip_all_tags(html_entity_decode((string) wc_price(max(0.0, $amount)), ENT_QUOTES, 'UTF-8')));
}

function papetarie_storefront_cart_shipping_summary_data(): array
{
    $fallback_message = __('Transportul se calculează la checkout.', 'papetarie-storefront');
    ...
    return [
        'available' => true,
        'message' => '',
        // Sursa: totalul de shipping calculat de WooCommerce cart, derivat din zonele și metodele configurate în admin.
        'value' => $shipping_total > 0.0
            ? papetarie_storefront_shipping_method_price_text($shipping_total)
            : __('Transport gratuit', 'papetarie-storefront'),
    ];
}

function papetarie_storefront_cart_products_subtotal_before_discounts(): float
{
    if (!function_exists('WC') || !WC()->cart) {
        return 0.0;
    }

    return max(0.0, (float) WC()->cart->get_subtotal());
}
...
{
    $remaining = papetarie_storefront_minimum_order_amount() - papetarie_storefront_cart_products_subtotal_before_discounts();
    return $remaining > 0 ? $remaining : 0.0;
}

function papetarie_storefront_cart_minimum_order_data(): array
{
    $minimum = papetarie_storefront_minimum_order_amount();
    $subtotal = papetarie_storefront_cart_products_subtotal_before_discounts();
    $remaining = max(0.0, $minimum - $subtotal);
    $blocked = $subtotal + 0.0001 < $minimum;
    ...
    return [
        ...
        'subtotal' => $subtotal,
        'remaining' => $remaining,
        'message' => $blocked ? papetarie_storefront_cart_minimum_order_message() : '',
    ];
}
...
function papetarie_storefront_cart_warning_state(): array
{
    ...
    $minimum_order_data = papetarie_storefront_cart_minimum_order_data();

    if (!empty($minimum_order_data['blocked'])) {
        return [
            'type' => 'minimum-order',
            'message' => (string) $minimum_order_data['message'],
            'visible' => true,
        ];
    }
...
function papetarie_storefront_cart_blocked_by_minimum_order(): bool
{
    ...
    return !empty(papetarie_storefront_cart_minimum_order_data()['blocked']);
}
...
function papetarie_storefront_enqueue_checkout_scripts(): void
{
    ...
    wp_localize_script(
        ...
        [
            ...
            'checkoutStandardAddressSnapshot' => function_exists('papetarie_storefront_checkout_standard_address_snapshot')
                ? papetarie_storefront_checkout_standard_address_snapshot()
                : [],
            'checkoutStandardAddressLines' => function_exists('papetarie_storefront_checkout_standard_address_lines')
                ? papetarie_storefront_checkout_standard_address_lines()
                : [],
...
function papetarie_storefront_get_checkout_notices_html(): string
{
    ...
    foreach ($notice_types as $notice_type) {
        ...
        if (empty($notices[$notice_type]) || !is_array($notices[$notice_type])) {
            continue;
        }
        ...
        foreach ($notices[$notice_type] as $notice) {
            ...
            $message = trim(wp_strip_all_tags((string) $notice['notice']));
            if (papetarie_storefront_is_checkout_coupon_success_notice($message)) {
                continue;
            }
        ...
        wc_get_template(
            ...
            [
                ...
                'notices' => array_filter($filtered_notices),
            ]
        );
    ...
    $html = trim(implode('', $filtered_html));
    if ($html !== '') {
        $html = preg_replace(
    ...
    return $has_visible_notice && '' !== trim(wp_strip_all_tags($html)) ? $html : '';
}

function papetarie_storefront_is_checkout_coupon_success_notice(string $notice): bool
{
    $normalized_notice = strtolower(remove_accents(trim(wp_strip_all_tags($notice))));
    ...
    $has_coupon_reference = strpos($normalized_notice, 'coupon') !== false
        || strpos($normalized_notice, 'cupon') !== false
        || strpos($normalized_notice, 'codul promotional') !== false
        || strpos($normalized_notice, 'cod promotional') !== false
        || strpos($normalized_notice, 'codul promo') !== false;
    $has_applied_reference = strpos($normalized_notice, 'applied') !== false || strpos($normalized_notice, 'aplicat') !== false;
    $has_success_reference = strpos($normalized_notice, 'success') !== false || strpos($normalized_notice, 'succes') !== false;

    return $has_coupon_reference && $has_applied_reference && $has_success_reference;
}

function papetarie_storefront_clear_checkout_coupon_success_notices(): void
{
    papetarie_storefront_clear_checkout_success_notices();
}

function papetarie_storefront_clear_checkout_success_notices(): void
{
    if (!function_exists('WC') || !WC() || !WC()->session) {
        return;
    }
    ...
    foreach ($all_notices['success'] as $notice) {
        ...
        $notice_text = trim(wp_strip_all_tags((string) $notice['notice']));
        if (papetarie_storefront_is_checkout_coupon_success_notice($notice_text)) {
            continue;
        }
...
function papetarie_storefront_is_coupon_flash_notice(string $notice): bool
{
    $normalized_notice = strtolower(remove_accents(trim(wp_strip_all_tags($notice))));
    ...
    return $has_coupon_reference;
}

function papetarie_storefront_queue_cart_coupon_toast(string $message, string $type = 'success'): void
{
    if (!function_exists('WC') || !WC() || !WC()->session) {
        return;
    }
    ...
    $toast_type = in_array($type, ['success', 'error', 'info', 'warning'], true) ? $type : 'success';
    WC()->session->set('pap_cart_coupon_toast', [
        'message' => $message,
        'type' => $toast_type,
    ]);
}

function papetarie_storefront_queue_coupon_action_toast_from_notice(string $notice, string $type): void
{
    $message = trim(wp_strip_all_tags($notice));
    if ($message === '' || !papetarie_storefront_is_coupon_flash_notice($message)) {
        return;
    }

    if (in_array($type, ['success', 'error', 'info', 'warning'], true)) {
        papetarie_storefront_queue_cart_coupon_toast(
            $type === 'success' ? __('Codul tău promoțional a fost activat cu succes', 'papetarie-storefront') : __('Codul tău promoțional a fost eliminat', 'papetarie-storefront'),
            $type
        );
    }
}

function papetarie_storefront_consume_cart_coupon_toast(): array
{
    if (!function_exists('WC') || !WC() || !WC()->session) {
        return [
            'message' => '',
    ...
    $toast = WC()->session->get('pap_cart_coupon_toast', []);
    if (!is_array($toast)) {
        $toast = [];
    }

    if (empty($toast['message'])) {
        $toast = papetarie_storefront_populate_coupon_toast_from_wc_notices();
    }
    ...
    return [
        ...
        'type' => isset($toast['type']) ? sanitize_key((string) $toast['type']) : 'success',
    ];
}

function papetarie_storefront_populate_coupon_toast_from_wc_notices(): array
{
    if (!function_exists('WC') || !WC() || !WC()->session) {
        return [];
    }
    ...
    foreach ($all_notices['success'] as $notice) {
        ...
        $notice_text = trim(wp_strip_all_tags((string) $notice['notice']));
        $normalized_notice = strtolower(remove_accents($notice_text));
        $is_coupon_notice = papetarie_storefront_is_coupon_flash_notice($notice_text)
            && (
                strpos($normalized_notice, 'applied successfully') !== false
                || strpos($normalized_notice, 'removed successfully') !== false
                || strpos($normalized_notice, 'a fost aplicat cu succes') !== false
                || strpos($normalized_notice, 'a fost eliminat') !== false
                || strpos($normalized_notice, 'a fost sters') !== false
            );
        ...
        if ($toast === []) {
            $toast = [
                'message' => (
                    strpos($normalized_notice, 'removed') !== false
                    || strpos($normalized_notice, 'eliminat') !== false
                    || strpos($normalized_notice, 'sters') !== false
                )
                    ? __('Codul tău promoțional a fost eliminat', 'papetarie-storefront')
                    : __('Codul tău promoțional a fost activat cu succes', 'papetarie-storefront'),
                'type' => 'success',
            ];
        }
    }
    ...
    WC()->session->set('wc_notices', empty($all_notices) ? null : $all_notices);
    WC()->session->set('pap_cart_coupon_toast', $toast);

    return $toast;
}

function papetarie_storefront_render_cart_coupon_toast_html(): string
{
    $toast = papetarie_storefront_consume_cart_coupon_toast();
    ...
    $notice_type = in_array(($toast['type'] ?? 'success'), ['success', 'error', 'info', 'warning'], true) ? (string) $toast['type'] : 'success';
    $icon = $notice_type === 'error'
        ? '!'
        : '✓';

    ob_start();
    ?>
    <div class="pap-toast-host pap-toast-host--cart" data-cart-toast-host>
      <div class="pap-toast pap-toast--cart pap-toast--<?php echo esc_attr($notice_type); ?> is-visible" data-cart-toast data-cart-toast-duration="3500" role="status" aria-live="polite">
        <span class="pap-toast__icon" aria-hidden="true"><?php echo esc_html($icon); ?></span>
        <span class="pap-toast__message" data-cart-toast-message><?php echo esc_html($message); ?></span>
        <button type="button" class="pap-toast__close" data-cart-toast-close aria-label="<?php esc_attr_e('Închide notificarea', 'papetarie-storefront'); ?>">×</button>
      </div>
    </div>
    <?php
...
function papetarie_storefront_capture_coupon_applied_toast(string $coupon_code): void
{
    if (trim($coupon_code) === '') {
        return;
    }

    papetarie_storefront_queue_cart_coupon_toast(__('Codul tău promoțional a fost activat cu succes', 'papetarie-storefront'), 'success');
}
add_action('woocommerce_applied_coupon', 'papetarie_storefront_capture_coupon_applied_toast', 20, 1);

function papetarie_storefront_capture_coupon_removed_toast(string $coupon_code): void
{
    if (trim($coupon_code) === '') {
        return;
    }

    papetarie_storefront_queue_cart_coupon_toast(__('Codul tău promoțional a fost eliminat', 'papetarie-storefront'), 'success');
}
add_action('woocommerce_removed_coupon', 'papetarie_storefront_capture_coupon_removed_toast', 20, 1);
...
function papetarie_storefront_get_checkout_shipping_methods_card_html(): string
{
    ...
    foreach ($packages as $package) {
        if (!empty($package['rates']) && is_array($package['rates'])) {
            ...
            $total_shipping_rates += count($package['rates']);
        }
    }
    ...
    if ($needs_shipping && !$has_shipping_method) {
        $shipping_notice = !empty($packages)
            ? __('Nu există metode de transport disponibile pentru adresa selectată.', 'papetarie-storefront')
            : __('Completează adresa de livrare pentru a calcula transportul.', 'papetarie-storefront');
    }
    ...
    <section class="pap-checkout-card pap-checkout-card--shipping-methods pap-checkout-step pap-checkout-step--shipping-methods is-step-<?php echo esc_attr($step_state); ?>" data-pap-checkout-section="shipping-methods" data-pap-checkout-step="shipping-methods" data-pap-step-state="<?php echo esc_attr($step_state); ?>" aria-disabled="<?php echo esc_attr($step_state === 'disabled' ? 'true' : 'false'); ?>">
      ...
      <div class="pap-checkout-step__body"<?php echo $body_hidden_attr; ?>>
        ...
        <?php if ($needs_shipping && $has_shipping_method) : ?>
          <?php if ($total_shipping_rates === 1) : ?>
            <?php
            $featured_package_index = 0;
            $featured_package = $packages[0] ?? [];
            $featured_rates = isset($featured_package['rates']) && is_array($featured_package['rates']) ? $featured_package['rates'] : [];
            $featured_rate = reset($featured_rates);
            if ($featured_rate instanceof WC_Shipping_Rate) :
                $featured_rate_key = key($featured_rates);
                $featured_rate_label = papetarie_storefront_shipping_method_label_text($featured_rate);
                $featured_rate_description = trim((string) $featured_rate->get_description());
                $featured_rate_cost_value = max(0.0, (float) $featured_rate->get_cost());
                $featured_rate_cost = $featured_rate_cost_value > 0
                    ? papetarie_storefront_shipping_method_price_text($featured_rate_cost_value)
                    : __('Transport gratuit', 'papetarie-storefront');
                $featured_input_id = sprintf('shipping_method_%d_%s', $featured_package_index, sanitize_title((string) $featured_rate_key));
                $featured_is_selected = function_exists('wc_get_chosen_shipping_method_for_package')
                    ? (string) wc_get_chosen_shipping_method_for_package($featured_package_index, $featured_package) === (string) $featured_rate_key
                    : true;
                ?>
                <div class="pap-checkout-shipping-feature">
                  <label for="<?php echo esc_attr($featured_input_id); ?>" class="pap-checkout-shipping-feature__card<?php echo $featured_is_selected ? ' is-selected' : ''; ?>">
                    ...
                    <div class="pap-checkout-shipping-feature__content">
                      <div class="pap-checkout-shipping-feature__title"><?php echo esc_html($featured_rate_label); ?></div>
                      <div class="pap-checkout-shipping-feature__description">
                        <?php echo esc_html($featured_rate_description !== '' ? $featured_rate_description : __('Livrare în 1–3 zile lucrătoare', 'papetarie-storefront')); ?>
                      </div>
                    </div>
                    ...
                    <input
                      type="radio"
                      class="shipping_method input-radio pap-checkout-shipping-feature__input"
                      name="<?php echo esc_attr(sprintf('shipping_method[%d]', (int) $featured_package_index)); ?>"
                      id="<?php echo esc_attr($featured_input_id); ?>"
                      value="<?php echo esc_attr((string) $featured_rate_key); ?>"
                      data-index="<?php echo esc_attr((int) $featured_package_index); ?>"
                      <?php checked($featured_is_selected); ?>
                    >
                  </label>
                </div>
                <?php
          ...
          <?php else : ?>
            <div class="pap-checkout-shipping-method-groups">
              ...
              $package_name = in_array(mb_strtolower(trim($package_name)), ['shipment', 'shipment 1', 'shipment 2'], true) ? '' : $package_name;
              ...
              <div class="pap-checkout-shipping-package">
                <?php foreach ($available_methods as $rate_key => $rate) : ?>
                  ...
                  $rate_label = papetarie_storefront_shipping_method_label_text($rate);
                  $rate_description = trim((string) $rate->get_description());
                  $rate_cost_value = max(0.0, (float) $rate->get_cost());
                  $rate_cost = $rate_cost_value > 0
                      ? papetarie_storefront_shipping_method_price_text($rate_cost_value)
                      : __('Transport gratuit', 'papetarie-storefront');
                  $is_selected = (string) $chosen_method === (string) $rate_key;
                  ?>
                  <div class="pap-checkout-shipping-method<?php echo $is_selected ? ' is-selected pap-checkout-shipping-summary' : ''; ?>">
                    <label for="<?php echo esc_attr($input_id); ?>">
                      ...
                      <span class="pap-checkout-shipping-summary__copy">
                        <span class="pap-checkout-shipping-summary__title"><?php echo esc_html($rate_label); ?></span>
                        <?php if ($rate_description !== '') : ?>
                          <span class="pap-checkout-shipping-summary__meta"><?php echo esc_html($rate_description); ?></span>
                        <?php endif; ?>
                      </span>
                      <span class="pap-checkout-shipping-summary__price"><?php echo esc_html($rate_cost); ?></span>
                    </label>
                  </div>
                <?php endforeach; ?>
              </div>
    ...
    <div class="pap-checkout-shipping-products<?php echo $has_more_products ? ' pap-has-more-products' : ''; ?>" data-pap-checkout-section="checkout-products">
      <div class="pap-checkout-shipping-products__head">
        <h3>
          ...
          <span><?php echo esc_html(sprintf(_n('(%d produs)', '(%d produse)', $visible_item_count, 'papetarie-storefront'), $visible_item_count)); ?></span>
        </h3>
      </div>
      <div class="pap-checkout-product-list" role="list" aria-label="<?php esc_attr_e('Produse din comandă', 'papetarie-storefront'); ?>">
        ...
        <div class="pap-checkout-products-toggle-wrap">
          <button
            type="button"
            class="pap-secondary-action pap-checkout-products-toggle"
...
function papetarie_storefront_checkout_guest_shipping_snapshot(): array
{
    ...
    $city = trim((string) ($snapshot['city'] ?? $snapshot['#shipping_city'] ?? $snapshot['billing_city'] ?? $snapshot['shipping_city'] ?? ''));
    $state = trim((string) ($snapshot['state'] ?? $snapshot['#shipping_state'] ?? $snapshot['billing_state'] ?? $snapshot['shipping_state'] ?? ''));
    $postcode = trim((string) ($snapshot['#shipping_postcode'] ?? $snapshot['postcode'] ?? ''));
    $snapshot['city'] = $city;
    $snapshot['state'] = $state;
    $snapshot['#billing_city'] = $city;
    $snapshot['#billing_state'] = $state;
    $snapshot['#shipping_city'] = $city;
    $snapshot['#shipping_state'] = $state;
    $snapshot['billing_city'] = $city;
    $snapshot['billing_state'] = $state;
    $snapshot['shipping_city'] = $city;
    $snapshot['shipping_state'] = $state;
    ...
    $snapshot['billing_postcode'] = $postcode;
    $snapshot['shipping_postcode'] = $postcode;
...
function papetarie_storefront_checkout_guest_shipping_summary_lines(): array
{
    ...
    $city = ($snapshot['city'] ?? '') ?: ($snapshot['#shipping_city'] ?? '') ?: ($snapshot['billing_city'] ?? '') ?: trim((string) $checkout->get_value('shipping_city')) ?: trim((string) $checkout->get_value('billing_city'));
    $state = ($snapshot['state'] ?? '') ?: ($snapshot['#shipping_state'] ?? '') ?: ($snapshot['billing_state'] ?? '') ?: trim((string) $checkout->get_value('shipping_state')) ?: trim((string) $checkout->get_value('billing_state'));
    $postcode = ($snapshot['postcode'] ?? '') ?: ($snapshot['#shipping_postcode'] ?? '') ?: ($snapshot['billing_postcode'] ?? '') ?: trim((string) $checkout->get_value('shipping_postcode')) ?: trim((string) $checkout->get_value('billing_postcode'));
    $order_comments = ($snapshot['#order_comments'] ?? '') ?: trim((string) $checkout->get_value('order_comments'));

    $lines = [];
    $full_name = trim($first_name . ' ' . $last_name);
    $address_line = trim(implode(', ', array_filter([$address_1, $address_2])));
    $city_line = papetarie_storefront_checkout_build_location_line($city, $state, $postcode);
    ...
    if ($order_comments !== '') {
        $lines[] = 'Observații: ' . $order_comments;
    }
...
function papetarie_storefront_checkout_standard_address_snapshot(): array
{
    if (
        ...
        && function_exists('papetarie_storefront_address_book_checkout_has_temporary_address')
        && papetarie_storefront_address_book_checkout_has_temporary_address()
        && function_exists('papetarie_storefront_address_book_checkout_temporary_snapshot')
        && function_exists('papetarie_storefront_address_book_checkout_temporary_snapshot_is_valid')
        && papetarie_storefront_address_book_checkout_temporary_snapshot_is_valid()
    ) {
        ...
        if (is_array($temporary_snapshot) && papetarie_storefront_checkout_snapshot_has_values($temporary_snapshot)) {
            return [
                ...
                'billing_email' => trim((string) ($temporary_snapshot['#billing_email'] ?? '')),
                'billing_phone' => trim((string) ($temporary_snapshot['#billing_phone'] ?? '')),
                'billing_country' => 'RO',
                'billing_state' => trim((string) ($temporary_snapshot['#shipping_state'] ?? '')),
                'billing_city' => trim((string) ($temporary_snapshot['#shipping_city'] ?? '')),
                'billing_postcode' => trim((string) ($temporary_snapshot['#shipping_postcode'] ?? '')),
                'billing_address_1' => trim((string) ($temporary_snapshot['#shipping_address_1'] ?? '')),
                'billing_address_2' => '',
                'shipping_first_name' => trim((string) ($temporary_snapshot['#billing_first_name'] ?? '')),
                'shipping_last_name' => trim((string) ($temporary_snapshot['#billing_last_name'] ?? '')),
                'shipping_phone' => trim((string) ($temporary_snapshot['#billing_phone'] ?? '')),
                'shipping_country' => 'RO',
                'shipping_state' => trim((string) ($temporary_snapshot['#shipping_state'] ?? '')),
                'shipping_city' => trim((string) ($temporary_snapshot['#shipping_city'] ?? '')),
                'shipping_postcode' => trim((string) ($temporary_snapshot['#shipping_postcode'] ?? '')),
                'shipping_address_1' => trim((string) ($temporary_snapshot['#shipping_address_1'] ?? '')),
                'shipping_address_2' => '',
                'order_comments' => trim((string) ($temporary_snapshot['#order_comments'] ?? '')),
            ];
        }
    }
    ...
    if (function_exists('is_user_logged_in') && is_user_logged_in()) {
        ...
        if (is_array($current_order_snapshot) && papetarie_storefront_checkout_snapshot_has_values($current_order_snapshot)) {
            return [
                ...
                'billing_email' => trim((string) ($current_order_snapshot['#billing_email'] ?? '')),
                'billing_phone' => trim((string) ($current_order_snapshot['#billing_phone'] ?? '')),
                'billing_country' => 'RO',
                'billing_state' => trim((string) ($current_order_snapshot['#shipping_state'] ?? $current_order_snapshot['#billing_state'] ?? '')),
                'billing_city' => trim((string) ($current_order_snapshot['#shipping_city'] ?? $current_order_snapshot['#billing_city'] ?? '')),
                'billing_postcode' => trim((string) ($current_order_snapshot['postcode'] ?? $current_order_snapshot['#shipping_postcode'] ?? $current_order_snapshot['#billing_postcode'] ?? '')),
                'billing_address_1' => trim((string) ($current_order_snapshot['#shipping_address_1'] ?? $current_order_snapshot['#billing_address_1'] ?? '')),
                'billing_address_2' => trim((string) ($current_order_snapshot['#shipping_address_2'] ?? $current_order_snapshot['#billing_address_2'] ?? '')),
                'shipping_first_name' => trim((string) ($current_order_snapshot['#billing_first_name'] ?? '')),
                'shipping_last_name' => trim((string) ($current_order_snapshot['#billing_last_name'] ?? '')),
                'shipping_phone' => trim((string) ($current_order_snapshot['#billing_phone'] ?? '')),
                'shipping_country' => 'RO',
                'shipping_state' => trim((string) ($current_order_snapshot['#shipping_state'] ?? $current_order_snapshot['#billing_state'] ?? '')),
                'shipping_city' => trim((string) ($current_order_snapshot['#shipping_city'] ?? $current_order_snapshot['#billing_city'] ?? '')),
                'shipping_postcode' => trim((string) ($current_order_snapshot['postcode'] ?? $current_order_snapshot['#shipping_postcode'] ?? $current_order_snapshot['#billing_postcode'] ?? '')),
                'shipping_address_1' => trim((string) ($current_order_snapshot['#shipping_address_1'] ?? $current_order_snapshot['#billing_address_1'] ?? '')),
                'shipping_address_2' => trim((string) ($current_order_snapshot['#shipping_address_2'] ?? $current_order_snapshot['#billing_address_2'] ?? '')),
                'order_comments' => trim((string) ($current_order_snapshot['#order_comments'] ?? '')),
            ];
        }
...
function papetarie_storefront_checkout_standard_address_lines(): array
{
    ...
    $first_name = $snapshot['shipping_first_name'] ?: $snapshot['billing_first_name'];
    $last_name = $snapshot['shipping_last_name'] ?: $snapshot['billing_last_name'];
    $phone = $snapshot['shipping_phone'] ?: $snapshot['billing_phone'];
    $email = $snapshot['billing_email'] ?? '';
    $address_1 = $snapshot['shipping_address_1'] ?: $snapshot['billing_address_1'];
    $address_2 = $snapshot['shipping_address_2'] ?: $snapshot['billing_address_2'];
    $city = $snapshot['shipping_city'] ?: $snapshot['billing_city'];
    $state = $snapshot['shipping_state'] ?: $snapshot['billing_state'];
    $postcode = $snapshot['shipping_postcode'] ?: $snapshot['billing_postcode'];
    $order_comments = trim((string) ($snapshot['order_comments'] ?? ''));

    $lines = [];
    $full_name = trim($first_name . ' ' . $last_name);
    $street_line = trim(implode(', ', array_filter([$address_1, $address_2])));
    $location_line = trim(implode(', ', array_filter([$city, $state, $postcode])));
...
function papetarie_storefront_checkout_standard_account_address_lines(): array
{
    ...
    $location_line = papetarie_storefront_checkout_build_location_line(
...
function papetarie_storefront_checkout_address_card_icon_svg(string $kind): string
{
    $icons = [
        ...
        'city' => '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" aria-hidden="true"><path d="M4 20V9l6-3v14" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M10 20V5l6 3v12" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M16 20v-7l4-2v9" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M2.5 20h19" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>',
        ...
        'note' => '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" aria-hidden="true"><path d="M5 4.5h14a1.5 1.5 0 0 1 1.5 1.5v8.5A1.5 1.5 0 0 1 19 16H9l-4 4v-4H5A1.5 1.5 0 0 1 3.5 14V6A1.5 1.5 0 0 1 5 4.5z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 8h10M7 11h7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>',
    ];

    return $icons[$kind] ?? $icons['location'];
}

function papetarie_storefront_checkout_normalize_location_parts(array $parts): array
{
    $normalized_parts = [];
    ...
    foreach ($parts as $index => $part) {
        ...
        $compact_value = preg_replace('/\s+/', ' ', $value);
        $compact_value = is_string($compact_value) ? trim($compact_value) : $value;
        if ($compact_value === '') {
            continue;
        }

        if (preg_match('/^[0-9]{6}$/', preg_replace('/\s+/', '', $compact_value))) {
            $has_later_postcode = false;
            foreach (array_slice($parts, $index + 1) as $later_part) {
                $later_value = trim((string) $later_part);
                if ($later_value === '') {
                    continue;
                }

                if (preg_match('/^[0-9]{6}$/', preg_replace('/\s+/', '', $later_value))) {
                    $has_later_postcode = true;
                    break;
                }
            }

            if ($has_later_postcode) {
                continue;
            }
        }

        $normalized_key = strtolower((string) remove_accents($compact_value));
        $already_added = false;
        foreach ($normalized_parts as $existing_part) {
            $existing_key = strtolower((string) remove_accents((string) $existing_part));
        ...
        $normalized_parts[] = $compact_value;
    }

    return $normalized_parts;
}

function papetarie_storefront_checkout_build_location_line(string $city, string $state, string $postcode): string
{
    $city = trim($city);
    $state = trim($state);
    $postcode = trim($postcode);

    if ($state !== '' && preg_match('/^[0-9]{6}$/', preg_replace('/\s+/', '', $state))) {
        if ($postcode === '' || $postcode === $state) {
            $postcode = $state;
        }
        $state = '';
    }

    $parts = papetarie_storefront_checkout_normalize_location_parts([$city, $state, $postcode]);
...
function papetarie_storefront_get_checkout_guest_shipping_summary_html(): string
{
    ...
    <div class="pap-checkout-address-card">
        <div class="pap-checkout-address-card__head">
            ...
            <div class="pap-checkout-address-card__meta">
                <button type="button" class="pap-secondary-action" data-pap-guest-shipping-edit>
                ...
                <?php foreach ($lines as $index => $line) : ?>
                    <p class="address-summary-row">
                        <span class="pap-checkout-address-card__icon address-summary-icon" aria-hidden="true">
                            <?php echo papetarie_storefront_checkout_address_card_icon_svg(0 === $index ? 'location' : (1 === $index ? 'city' : (2 === $index ? 'phone' : (3 === $index ? 'email' : 'note')))); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                        </span>
...
function papetarie_storefront_get_checkout_auth_shipping_summary_html(): string
{
    ...
    <div class="pap-checkout-address-card">
        <div class="pap-checkout-address-card__head">
            ...
            <div class="pap-checkout-address-card__meta">
                <button type="button" class="pap-secondary-action" data-pap-auth-temporary-edit>
                    <?php esc_html_e('Modifică', 'papetarie-storefront'); ?>
                </button>
            </div>
        </div>
...
function papetarie_storefront_checkout_step_state(string $step): string
{
    ...
    $has_standard_address = papetarie_storefront_checkout_standard_address_has_content();
    $normalized_step = sanitize_key($step);

    if ($normalized_step === 'shipping-address') {
        return $has_standard_address ? 'complete' : 'active';
    }

    if ($normalized_step === 'shipping-methods') {
        return $has_standard_address ? 'active' : 'disabled';
    }
...
function papetarie_storefront_romania_counties(): array
{
    ...
    $dataset_path = get_stylesheet_directory() . '/data/siruta-counties.json';
...
function papetarie_storefront_romania_localities_by_county(): array
{
    ...
    $dataset_path = get_stylesheet_directory() . '/data/siruta-localities-by-county.json';
    if (!file_exists($dataset_path)) {
    ...
    foreach ($counties as $county_code => $county_name) {
        ...
        foreach ($county_localities as $city) {
            $raw_city = (string) $city;
            $city = papetarie_storefront_normalize_locality_label($raw_city);
            $city_key = papetarie_storefront_normalize_locality_key($city);
            if ($city === '' || $city_key === '') {
                continue;
            }

            $is_prefixed = papetarie_storefront_locality_has_prefix($raw_city);
            if (!isset($normalized_localities[$city_key])) {
                $normalized_localities[$city_key] = [
                    'label' => $city,
            ...
            if (!empty($normalized_localities[$city_key]['prefixed']) && !$is_prefixed) {
                $normalized_localities[$city_key] = [
                    'label' => $city,
                    'prefixed' => false,
                ];
        ...
        $county_localities = array_map(
            static fn (array $entry): string => (string) ($entry['label'] ?? ''),
            array_values($normalized_localities)
        );
        $county_localities = array_values(array_filter($county_localities, static fn (string $city): bool => $city !== ''));
        usort($county_localities, static function (string $left, string $right): int {
            return strcmp(papetarie_storefront_normalize_locality_key($left), papetarie_storefront_normalize_locality_key($right));
        });

        $ordered[$county_code] = $county_localities;
    }
...
{
    $county_code = strtoupper(sanitize_key($county_code));
    $cities_by_county = papetarie_storefront_romania_cities_by_county();
...
{
    $city = papetarie_storefront_normalize_locality_key($city);

    return $city;
}

function papetarie_storefront_locality_has_prefix(string $locality): bool
{
    $locality = trim(preg_replace('/\s+/', ' ', $locality));
    $locality = str_replace(['Ţ', 'ţ', 'Ş', 'ş'], ['Ț', 'ț', 'Ș', 'ș'], $locality);
    $locality = strtolower(remove_accents($locality));
    $locality = preg_replace('/[^a-z0-9]+/u', ' ', $locality) ?? $locality;
    $locality = trim(preg_replace('/\s+/', ' ', $locality) ?? $locality);

    foreach (['judetul ', 'municipiul ', 'orasul ', 'oras ', 'comuna ', 'satul ', 'sectorul '] as $prefix) {
        if (strpos($locality, $prefix) === 0) {
            return true;
        }
    }
...
function papetarie_storefront_normalize_locality_label(string $locality, bool $title_case = true): string
{
    $locality = trim(preg_replace('/\s+/', ' ', $locality));
    $locality = str_replace(['Ţ', 'ţ', 'Ş', 'ş'], ['Ț', 'ț', 'Ș', 'ș'], $locality);
    $locality = preg_replace(
        '/^(JUDEȚUL|JUDEŢUL|JUDETUL|MUNICIPIUL|ORAȘUL|ORAŞUL|ORASUL|ORAȘ|ORAŞ|ORAS|COMUNA|SATUL|SECTORUL)\s+/iu',
        '',
        $locality
    ) ?? $locality;
    ...
    if (function_exists('mb_convert_case') && $title_case) {
        $locality = mb_convert_case($locality, MB_CASE_TITLE, 'UTF-8');
    } elseif ($title_case) {
        $locality = ucwords(strtolower($locality));
    }

    if (strcasecmp(remove_accents($locality), 'bucuresti') === 0) {
        return 'București';
    }
...
function papetarie_storefront_normalize_locality_key(string $locality): string
{
    $locality = trim(preg_replace('/\s+/', ' ', $locality));
    $locality = str_replace(['Ţ', 'ţ', 'Ş', 'ş'], ['Ț', 'ț', 'Ș', 'ș'], $locality);
    $locality = strtolower(remove_accents($locality));
    $locality = preg_replace(
        '/^(judetul|municipiul|orasul|oras|comuna|satul|sectorul)\s+/u',
        '',
        $locality
    ) ?? $locality;
    $locality = preg_replace('/[^a-z0-9]+/u', ' ', $locality) ?? $locality;
...
function papetarie_storefront_should_show_tax_rows(): bool
{
    return (bool) apply_filters('papetarie_storefront_show_tax_rows', false);
}
...
function papetarie_storefront_render_cart_summary_html(string $notice_html = ''): string
{
    ...
    $coupon_codes = $cart ? array_values((array) $cart->get_applied_coupons()) : [];
    $coupon_inline_error = papetarie_storefront_get_coupon_inline_error();
    $minimum_order_data = papetarie_storefront_cart_minimum_order_data();
    $is_checkout_blocked = !empty($minimum_order_data['blocked']) || papetarie_storefront_cart_warning_state()['type'] !== 'none' || papetarie_storefront_cart_has_stock_insufficient_items();
    $shipping_summary = papetarie_storefront_cart_shipping_summary_data();
    $show_shipping_row = !empty($shipping_summary['available']) || !empty($shipping_summary['message']);
    ...
    <div class="pap-cart-summary-card" data-cart-summary-card>
      ...
      <div class="pap-cart-totals">
        ...
        <?php if ($show_shipping_row) : ?>
          <div class="pap-cart-totals-row pap-cart-totals-row--shipping">
            ...
            <strong data-cart-summary-shipping>
              <?php if (!empty($shipping_summary['available'])) : ?>
                <?php echo esc_html((string) $shipping_summary['value']); ?>
              <?php else : ?>
                <span class="pap-cart-totals-row__note"><?php echo esc_html((string) $shipping_summary['message']); ?></span>
              <?php endif; ?>
            </strong>
          </div>
        <?php endif; ?>
        ...
        <?php foreach ($coupon_codes as $coupon_code) : ?>
          ...
          ?>
          <div class="pap-cart-totals-row pap-cart-totals-row--discount coupon-<?php echo esc_attr(sanitize_title($coupon_code)); ?>" data-coupon-code="<?php echo esc_attr($coupon_code); ?>">
            <span><?php echo esc_html(sprintf(__('Reducere (%s)', 'papetarie-storefront'), wc_format_coupon_code($coupon_code))); ?></span>
            <strong data-cart-summary-discount><?php echo wp_kses_post(wc_price(0 - $coupon_discount_amount)); ?></strong>
          </div>
        <?php endforeach; ?>

        <?php if (!$has_coupon_discount_rows && $cart && ((float) $cart->get_discount_total() > 0.0001 || (float) $cart->get_discount_tax() > 0.0001)) : ?>
          <?php $discount_amount = max(0.0, (float) $cart->get_discount_total() + (float) $cart->get_discount_tax()); ?>
          <div class="pap-cart-totals-row pap-cart-totals-row--discount cart-discount--summary">
            <span><?php esc_html_e('Reducere', 'papetarie-storefront'); ?></span>
            <strong data-cart-summary-discount><?php echo wp_kses_post(wc_price(0 - $discount_amount)); ?></strong>
          </div>
        <?php endif; ?>
      </div>
      ...
      <a
        ...
        data-cart-checkout
        data-cart-minimum-order-blocked="<?php echo esc_attr(!empty($minimum_order_data['blocked']) ? '1' : '0'); ?>"
...
function papetarie_storefront_icon(string $name): string
{
    $icons = [
        ...
        'cookie' => '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.5 12.3c-1.6.2-3-1-3.1-2.6-1.5.3-2.9-.8-3-2.3-1.7.4-3.4-.7-3.7-2.4A9 9 0 1 0 20.5 12.3Z" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/><circle cx="9" cy="10.5" r="1.1" fill="currentColor"/><circle cx="8" cy="15" r="1.1" fill="currentColor"/><circle cx="13" cy="16" r="1.1" fill="currentColor"/><circle cx="14.5" cy="12.5" r="1.1" fill="currentColor"/></svg>',
...
function papetarie_storefront_is_admin_destination(string $url): bool
{
    if ('' === $url) {
        return false;
    }
    ...
    return str_contains($path, '/wp-admin')
        ...
        || str_contains($path, 'admin.php');
}

function papetarie_storefront_login_redirect_resolver(string $redirect_to, string $requested_redirect_to, $user): string
{
    if ($user instanceof WP_Error || !($user instanceof WP_User)) {
        return $redirect_to;
    }

    $is_admin_destination = papetarie_storefront_is_admin_destination($requested_redirect_to)
        || papetarie_storefront_is_admin_destination($redirect_to);

    if (!$is_admin_destination) {
        return $redirect_to;
    }
...
add_filter('login_redirect', 'papetarie_storefront_login_redirect_resolver', PHP_INT_MAX, 3);
add_filter('woocommerce_login_redirect', static function (string $redirect_to, $user): string {
    if ($user instanceof WP_Error || !($user instanceof WP_User)) {
        return $redirect_to;
    }

    if (!papetarie_storefront_is_admin_destination($redirect_to)) {
        return $redirect_to;
    }
...
function papetarie_storefront_checkout_mirror_shipping_to_billing(array $data): array
{
    $shipping_to_billing_map = [
    ...
    $use_shipping_as_primary = !empty($data['ship_to_different_address']) || !empty($data['shipping_state']) || !empty($data['shipping_address_1']);

    foreach ($shipping_to_billing_map as $billing_key => $shipping_key) {
        $billing_value = isset($data[$billing_key]) ? trim((string) $data[$billing_key]) : '';
        $shipping_value = isset($data[$shipping_key]) ? trim((string) $data[$shipping_key]) : '';

        if ($use_shipping_as_primary && $shipping_value !== '') {
...
add_filter('woocommerce_checkout_posted_data', 'papetarie_storefront_checkout_mirror_shipping_to_billing', 10, 1);

function papetarie_storefront_checkout_normalize_county_value(string $value): string
{
    $value = trim($value);
    if ($value === '') {
        return '';
    ...
    $counties = papetarie_storefront_romania_counties();
    $normalized_key = strtoupper(sanitize_key($value));
    if (isset($counties[$normalized_key])) {
        return $normalized_key;
    }

    $value_key = papetarie_storefront_normalize_city_key($value);
    foreach ($counties as $county_code => $county_name) {
        if (papetarie_storefront_normalize_city_key((string) $county_name) === $value_key) {
            return (string) $county_code;
        }
    }
...
function papetarie_storefront_checkout_sync_customer_from_posted_data(array $posted_data): void
{
    if (!function_exists('WC') || !WC() || !(WC()->customer instanceof WC_Customer)) {
        return;
    }
    ...
    $shipping_country = strtoupper(sanitize_text_field((string) ($posted_data['shipping_country'] ?? $posted_data['billing_country'] ?? $billing_country)));
    if ($shipping_country === '') {
        $shipping_country = $billing_country !== '' ? $billing_country : 'RO';
    }
    $billing_state = papetarie_storefront_checkout_normalize_county_value((string) ($posted_data['billing_state'] ?? $posted_data['shipping_state'] ?? ''));
    $shipping_state = papetarie_storefront_checkout_normalize_county_value((string) ($posted_data['shipping_state'] ?? $posted_data['billing_state'] ?? $billing_state));
    $billing_city = sanitize_text_field((string) ($posted_data['billing_city'] ?? $posted_data['shipping_city'] ?? ''));
    $shipping_city = sanitize_text_field((string) ($posted_data['shipping_city'] ?? $posted_data['billing_city'] ?? $billing_city));
    $billing_postcode = sanitize_text_field((string) ($posted_data['billing_postcode'] ?? $posted_data['shipping_postcode'] ?? $posted_data['postcode'] ?? ''));
    $shipping_postcode = sanitize_text_field((string) ($posted_data['shipping_postcode'] ?? $posted_data['billing_postcode'] ?? $posted_data['postcode'] ?? $billing_postcode));
    $billing_address_1 = sanitize_text_field((string) ($posted_data['billing_address_1'] ?? $posted_data['shipping_address_1'] ?? ''));
    $shipping_address_1 = sanitize_text_field((string) ($posted_data['shipping_address_1'] ?? $posted_data['billing_address_1'] ?? $billing_address_1));
    $billing_address_2 = sanitize_text_field((string) ($posted_data['billing_address_2'] ?? $posted_data['shipping_address_2'] ?? ''));
    $shipping_address_2 = sanitize_text_field((string) ($posted_data['shipping_address_2'] ?? $posted_data['billing_address_2'] ?? $billing_address_2));
    $billing_company = sanitize_text_field((string) ($posted_data['billing_company'] ?? $posted_data['shipping_company'] ?? ''));
    $shipping_company = sanitize_text_field((string) ($posted_data['shipping_company'] ?? $posted_data['billing_company'] ?? $billing_company));
    ...
    $customer->set_billing_country($billing_country !== '' ? $billing_country : 'RO');
    ...
    $customer->set_shipping_first_name($billing_first_name !== '' ? $billing_first_name : $customer->get_shipping_first_name());
    $customer->set_shipping_last_name($billing_last_name !== '' ? $billing_last_name : $customer->get_shipping_last_name());
    $customer->set_shipping_phone($billing_phone !== '' ? $billing_phone : $customer->get_shipping_phone());
    $customer->set_shipping_country($shipping_country !== '' ? $shipping_country : ($billing_country !== '' ? $billing_country : 'RO'));
    ...
    if (method_exists($customer, 'save_data')) {
        $customer->save_data();
    }
}

function papetarie_storefront_checkout_sync_customer_from_request(?string $post_data = null): void
{
    $posted_data = [];

    if (is_string($post_data) && trim($post_data) !== '') {
        parse_str(wp_unslash($post_data), $posted_data);
    ...
    papetarie_storefront_checkout_sync_customer_from_posted_data($posted_data);
}

add_action('woocommerce_checkout_update_order_review', 'papetarie_storefront_checkout_sync_customer_from_request', 5, 1);
add_action('woocommerce_before_checkout_process', 'papetarie_storefront_checkout_sync_customer_from_request', 5);

function papetarie_storefront_checkout_customer_restore_snapshot_key(): string
{
    return 'papetarie_checkout_customer_restore_snapshot';
}

function papetarie_storefront_checkout_customer_restore_pending_key(): string
{
    return 'papetarie_checkout_customer_restore_pending';
}

function papetarie_storefront_checkout_should_persist_address_to_account(): bool
{
    if (!is_user_logged_in()) {
        return false;
    }

    $save_checkbox = isset($_POST['pap_save_address_for_future']) ? sanitize_text_field(wp_unslash((string) $_POST['pap_save_address_for_future'])) : '';
...
function papetarie_storefront_checkout_capture_customer_snapshot(): void
{
    ...
    if (papetarie_storefront_checkout_should_persist_address_to_account()) {
        $session = WC()->session;
        if ($session) {
            $session->set(papetarie_storefront_checkout_customer_restore_pending_key(), false);
            $session->set(papetarie_storefront_checkout_customer_restore_snapshot_key(), []);
    ...
    $session = WC()->session;
    if ($session) {
        $pending = (bool) $session->get(papetarie_storefront_checkout_customer_restore_pending_key(), false);
        $existing_snapshot = $session->get(papetarie_storefront_checkout_customer_restore_snapshot_key(), []);
        if ($pending || (is_array($existing_snapshot) && !empty($existing_snapshot))) {
            return;
        }
    }
    ...
    foreach ($snapshot as $key => $value) {
        $snapshot[$key] = sanitize_text_field($value);
    }

    if ($session) {
        $session->set(papetarie_storefront_checkout_customer_restore_snapshot_key(), $snapshot);
        $session->set(papetarie_storefront_checkout_customer_restore_pending_key(), true);
        if (method_exists($session, 'save_data')) {
            $session->save_data();
        }
    }
...
function papetarie_storefront_checkout_restore_customer_snapshot(): void
{
    if (!function_exists('WC') || !WC() || !(WC()->customer instanceof WC_Customer) || !is_user_logged_in()) {
        return;
    }
    ...
    $snapshot = $session->get(papetarie_storefront_checkout_customer_restore_snapshot_key(), []);
    ...
    $user = get_userdata($user_id);
    $user_email = isset($snapshot['user_email']) ? sanitize_email((string) $snapshot['user_email']) : '';

    update_user_meta($user_id, 'first_name', sanitize_text_field((string) ($snapshot['first_name'] ?? '')));
    update_user_meta($user_id, 'last_name', sanitize_text_field((string) ($snapshot['last_name'] ?? '')));
    update_user_meta($user_id, 'billing_first_name', sanitize_text_field((string) ($snapshot['billing_first_name'] ?? '')));
    update_user_meta($user_id, 'billing_last_name', sanitize_text_field((string) ($snapshot['billing_last_name'] ?? '')));
    update_user_meta($user_id, 'billing_email', sanitize_email((string) ($snapshot['billing_email'] ?? '')));
    update_user_meta($user_id, 'billing_phone', sanitize_text_field((string) ($snapshot['billing_phone'] ?? '')));
    update_user_meta($user_id, 'billing_country', sanitize_text_field((string) ($snapshot['billing_country'] ?? 'RO')));
    update_user_meta($user_id, 'billing_state', sanitize_text_field((string) ($snapshot['billing_state'] ?? '')));
    update_user_meta($user_id, 'billing_city', sanitize_text_field((string) ($snapshot['billing_city'] ?? '')));
    update_user_meta($user_id, 'billing_postcode', sanitize_text_field((string) ($snapshot['billing_postcode'] ?? '')));
    update_user_meta($user_id, 'billing_address_1', sanitize_text_field((string) ($snapshot['billing_address_1'] ?? '')));
    update_user_meta($user_id, 'billing_address_2', sanitize_text_field((string) ($snapshot['billing_address_2'] ?? '')));
    update_user_meta($user_id, 'billing_company', sanitize_text_field((string) ($snapshot['billing_company'] ?? '')));
    update_user_meta($user_id, 'shipping_first_name', sanitize_text_field((string) ($snapshot['shipping_first_name'] ?? '')));
    update_user_meta($user_id, 'shipping_last_name', sanitize_text_field((string) ($snapshot['shipping_last_name'] ?? '')));
    update_user_meta($user_id, 'shipping_phone', sanitize_text_field((string) ($snapshot['shipping_phone'] ?? '')));
    update_user_meta($user_id, 'shipping_country', sanitize_text_field((string) ($snapshot['shipping_country'] ?? 'RO')));
    update_user_meta($user_id, 'shipping_state', sanitize_text_field((string) ($snapshot['shipping_state'] ?? '')));
    update_user_meta($user_id, 'shipping_city', sanitize_text_field((string) ($snapshot['shipping_city'] ?? '')));
    update_user_meta($user_id, 'shipping_postcode', sanitize_text_field((string) ($snapshot['shipping_postcode'] ?? '')));
    update_user_meta($user_id, 'shipping_address_1', sanitize_text_field((string) ($snapshot['shipping_address_1'] ?? '')));
    update_user_meta($user_id, 'shipping_address_2', sanitize_text_field((string) ($snapshot['shipping_address_2'] ?? '')));
    update_user_meta($user_id, 'shipping_company', sanitize_text_field((string) ($snapshot['shipping_company'] ?? '')));

    if ($user instanceof WP_User && $user_email !== '' && $user->user_email !== $user_email) {
        wp_update_user([
            'ID' => $user_id,
    ...
    $customer = WC()->customer;
    $customer->set_first_name(sanitize_text_field((string) ($snapshot['first_name'] ?? '')));
    $customer->set_last_name(sanitize_text_field((string) ($snapshot['last_name'] ?? '')));
    $customer->set_email($user_email);
    $customer->set_billing_first_name(sanitize_text_field((string) ($snapshot['billing_first_name'] ?? '')));
    $customer->set_billing_last_name(sanitize_text_field((string) ($snapshot['billing_last_name'] ?? '')));
    $customer->set_billing_email(sanitize_email((string) ($snapshot['billing_email'] ?? '')));
    $customer->set_billing_phone(sanitize_text_field((string) ($snapshot['billing_phone'] ?? '')));
    $customer->set_billing_country(sanitize_text_field((string) ($snapshot['billing_country'] ?? 'RO')));
    $customer->set_billing_state(sanitize_text_field((string) ($snapshot['billing_state'] ?? '')));
    $customer->set_billing_city(sanitize_text_field((string) ($snapshot['billing_city'] ?? '')));
    $customer->set_billing_postcode(sanitize_text_field((string) ($snapshot['billing_postcode'] ?? '')));
    $customer->set_billing_address_1(sanitize_text_field((string) ($snapshot['billing_address_1'] ?? '')));
    $customer->set_billing_address_2(sanitize_text_field((string) ($snapshot['billing_address_2'] ?? '')));
    $customer->set_billing_company(sanitize_text_field((string) ($snapshot['billing_company'] ?? '')));
    $customer->set_shipping_first_name(sanitize_text_field((string) ($snapshot['shipping_first_name'] ?? '')));
    $customer->set_shipping_last_name(sanitize_text_field((string) ($snapshot['shipping_last_name'] ?? '')));
    $customer->set_shipping_phone(sanitize_text_field((string) ($snapshot['shipping_phone'] ?? '')));
    $customer->set_shipping_country(sanitize_text_field((string) ($snapshot['shipping_country'] ?? 'RO')));
    $customer->set_shipping_state(sanitize_text_field((string) ($snapshot['shipping_state'] ?? '')));
    $customer->set_shipping_city(sanitize_text_field((string) ($snapshot['shipping_city'] ?? '')));
    $customer->set_shipping_postcode(sanitize_text_field((string) ($snapshot['shipping_postcode'] ?? '')));
    $customer->set_shipping_address_1(sanitize_text_field((string) ($snapshot['shipping_address_1'] ?? '')));
    $customer->set_shipping_address_2(sanitize_text_field((string) ($snapshot['shipping_address_2'] ?? '')));
    $customer->set_shipping_company(sanitize_text_field((string) ($snapshot['shipping_company'] ?? '')));
    $customer->save();
...
add_action('woocommerce_before_checkout_form', 'papetarie_storefront_checkout_capture_customer_snapshot', 1);
add_action('woocommerce_before_checkout_process', 'papetarie_storefront_checkout_capture_customer_snapshot', 1);
...
function papetarie_storefront_checkout_persist_snapshot_to_account(array $snapshot): void
{
    ...
    update_user_meta($user_id, 'billing_country', $billing_country !== '' ? $billing_country : 'RO');
    ...
    update_user_meta($user_id, 'shipping_country', $billing_country !== '' ? $billing_country : 'RO');
    ...
    $customer->set_shipping_country($billing_country !== '' ? $billing_country : 'RO');
...
function papetarie_storefront_checkout_mirror_shipping_to_order($order, array $data): void
{
    ...
    $shipping_country = sanitize_text_field((string) ($data['shipping_country'] ?? 'RO'));
    $shipping_state = sanitize_text_field((string) ($data['shipping_state'] ?? ''));
    $shipping_city = sanitize_text_field((string) ($data['shipping_city'] ?? ''));
    $shipping_postcode = sanitize_text_field((string) ($data['shipping_postcode'] ?? ''));
    $shipping_address_1 = sanitize_text_field((string) ($data['shipping_address_1'] ?? ''));
    $shipping_address_2 = sanitize_text_field((string) ($data['shipping_address_2'] ?? ''));

    if (trim((string) $order->get_billing_address_1()) === '' && $shipping_address_1 !== '') {
        $order->set_billing_country($shipping_country !== '' ? $shipping_country : 'RO');
...
add_action('woocommerce_checkout_create_order', 'papetarie_storefront_checkout_mirror_shipping_to_order', 5, 2);
...
function papetarie_storefront_checkout_validate(array $data, \WP_Error $errors): void
{
    ...
    $address_state = $shipping_state !== '' ? $shipping_state : $billing_state;
    ...
    if ($address_state === '') {
        ...
        if (!isset($counties[$address_state])) {
    ...
    } elseif ($address_state !== '' && isset($counties[$address_state])) {
        $billing_city_options = papetarie_storefront_checkout_city_options_for_county($address_state);
        $billing_city_key = papetarie_storefront_normalize_city_key($address_city);
    ...
    if ($address_line === '') {
        $errors->add('billing_address_1_required', __('Completează adresa.', 'papetarie-storefront'));
    }
